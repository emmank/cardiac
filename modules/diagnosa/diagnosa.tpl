...
$result .= $vl['datafields']['diagnosa']['title'];
...
foreach($vl['datafields'] as $yk => $lv){
    if($lv['type'] != 'hidden'){
        ...
        $result .= '<td align="right">' .ucwords (str_replace ("_"," ", $lv['title'])) . ' :</td>';
        $result .= '<td>';
        if($lv['type'] == 'select'){
            if($lv['readonly'] === true){
        ...
        } elseif($lv['type'] == 'textarea'){
//            $lv['value'] = explode('|', $lv['value']);
            if($lv['readonly'] === true){
//                foreach($lv['value'] as $vv){
//                    if(isset($koma)){$result .= ', ';}
                    $result .= $lv['value'];//[$vv]; $koma = 1;
//                } unset($koma);
            } else {
//                $htg = 0;
//                foreach($lv['theref'] as $kc => $isi){
                    $result .= '<textarea name="' . $yk . '" rows="4" cols="20"></textarea>';
//                    $result .= $isi . "<br />";
//                    $htg++;
//                } unset($htg);
            }
